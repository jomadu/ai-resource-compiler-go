...
func TestCursorCompiler_CompileRule(t *testing.T) {
	...
	result := results[0]
	if result.Path != "testRule.mdc" {
		t.Errorf("Path = %v, want testRule.mdc", result.Path)
	}

	if !strings.Contains(result.Content, "description: A test rule") {
		t.Error("Content missing MDC frontmatter description")
	}
	if !strings.Contains(result.Content, "alwaysApply: true") {
		t.Error("Content missing MDC frontmatter alwaysApply")
	}
	if !strings.Contains(result.Content, "**/*.ts") {
		t.Error("Content missing MDC frontmatter globs")
	}
...
func TestCursorCompiler_CompileRuleset(t *testing.T) {
	c := &CursorCompiler{}
	resource := &compiler.Resource{
		APIVersion: "ai-resource/draft",
		Kind:       "Ruleset",
	...
	if !contains(paths, "testRuleset_rule1.mdc") {
		t.Error("Missing testRuleset_rule1.mdc")
	}
	if !contains(paths, "testRuleset_rule2.mdc") {
		t.Error("Missing testRuleset_rule2.mdc")
	}
}

func TestCursorCompiler_CompilePrompt(t *testing.T) {
	c := &CursorCompiler{}
	resource := &compiler.Resource{
		APIVersion: "ai-resource/draft",
		Kind:       "Prompt",
...
func TestCursorCompiler_CompilePromptset(t *testing.T) {
	c := &CursorCompiler{}
	resource := &compiler.Resource{
		APIVersion: "ai-resource/draft",
		Kind:       "Promptset",
